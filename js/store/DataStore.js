...
const DataStore = (() => {
  ...
  const fetchers = new Map();  // key -> última função de busca conhecida (para refreshAll)
  ...
  async function revalidate(key, fetcher) {
    if (fetcher) fetchers.set(key, fetcher);
    try {
      const data = await (fetcher || fetchers.get(key))();
  ...
  /**
   * Atualiza AGORA todas as chaves atualmente conhecidas (usado pelo
   * botão "Atualizar agora" da barra superior). Roda em paralelo.
   */
  async function refreshAll() {
    const keys = Array.from(fetchers.keys());
    await Promise.all(keys.map((key) => revalidate(key)));
    return keys.length;
  }
  ...
  return { get, revalidate, subscribe, invalidate, clear, refreshAll };
})();
...
function useCachedQuery(key, fetcher, options = {}) {
  const { pollInterval = (window.APP_CONFIG?.POLL_INTERVAL || 300000), enabled = true } = options;
