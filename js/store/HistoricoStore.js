 ...
 * Anotações), o Progresso/Prazo Atuais (último registro em
 * Fiscalização) e o Responsável Atual (o mesmo critério do Status:
 * o registro mais recente entre Anotações E Fiscalização, comparando
 * as duas tabelas) — tudo a partir dos stores já carregados no
...
function calcularSituacaoObra(idObra) {
  ...
  // Responsável atual: compara as DUAS tabelas e usa o registro mais recente entre elas
  const dataAnotacao = ultimaAnotacao ? new Date(ultimaAnotacao['DataCriacaoRegistro']) : null;
  const dataFiscalizacao = ultimaFiscalizacao ? new Date(ultimaFiscalizacao['DataCriacaoRegistro']) : null;
  let responsavelAtual = '';
  if (dataAnotacao && (!dataFiscalizacao || dataAnotacao >= dataFiscalizacao)) {
    responsavelAtual = ultimaAnotacao['RESPONSÁVEL '] || '';
  } else if (dataFiscalizacao) {
    responsavelAtual = ultimaFiscalizacao['Responsável ?'] || '';
  }
...
function useObrasComSituacao() {
  ...
  const enriched = React.useMemo(() => obras.map((o) => {
    ...
    return {
      ...
      ResponsavelAtual: situacao.responsavelAtual
    };
