...
function ObraDetailPage({ navigate, params }) {
  ...
  const { data: res, loading, error, refresh: refetchDetail } = useCachedQuery(detailKey, () => ObrasAPI.get(params.idObra));
  ...
  return (
    <div className="fade-in">
      <div className="d-flex align-items-center gap-2 mb-3">
        ...
        <RefreshButton onRefresh={refetchDetail} label="Atualizar" />
      ...
      {tab === 'resumo' && (
        <div className="row g-3">
          <div className="col-lg-8">
            {FIELD_GROUPS.filter((g) => g.title.indexOf('Localização Geográfica') === -1).map((group) => (
              <div key={group.title} className="card border-0 shadow-sm mb-3"><div className="card-body">
                <h6 className="section-title">{group.title}</h6>
                <dl className="row mb-0">
                  {group.fields.map((f) => (
                    <React.Fragment key={f.key}>
                      <dt className="col-sm-4">{f.label}</dt>
                      <dd className="col-sm-8">{obra[f.key] !== undefined && obra[f.key] !== '' ? String(obra[f.key]) : '-'}</dd>
                    </React.Fragment>
                  ))}
                </dl>
              </div></div>
