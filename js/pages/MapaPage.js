 ...
 * ============================================================
 *  MapaPage.js
 *  Mapa com um pin por obra que tenha o campo LOCAL DA OBRA
 *  preenchido (formato "latitude,longitude"). Usa Leaflet +
 *  OpenStreetMap (gratuito, sem chave de API). Clicar num pin
 *  abre um popup com o resumo da obra e um link para o detalhe.
 * ============================================================
 */

function MapaPage({ navigate, params }) {
  const { obras, loading } = useObrasComSituacao();
  const mapRef = React.useRef(null);
  const mapInstance = React.useRef(null);
  const markersRef = React.useRef([]);

  const obrasComPin = React.useMemo(
    () => obras.filter((o) => parseLocalizacao(o['LOCAL DA OBRA'])),
    [obras]
  );
  ...
  React.useEffect(() => {
    ...
    obrasComPin.forEach((obra) => {
      const [lat, lng] = parseLocalizacao(obra['LOCAL DA OBRA']);
      const marker = window.L.marker([lat, lng]).addTo(mapInstance.current);
      const popupHtml = `
        <div style="min-width:200px;">
          <strong>${(obra['RUA'] || 'Obra ' + obra['CÓD. OBRA']).replace(/</g, '')}</strong>
  ...
  return (
    <div className="fade-in">
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <small className="text-muted">{obrasComPin.length} de {obras.length} obra(s) com localização cadastrada</small>
      </div>
      ...
      {!loading && obrasComPin.length === 0 && (
        <div className="mt-3">
          <EmptyState icon="bi-geo-alt" title="Nenhuma obra com localização cadastrada"
            subtitle="Edite uma obra e preencha o campo Local da Obra (manualmente ou usando o botão de GPS) para que ela apareça aqui." />
        </div>
      )}
    </div>
  );
...
function MapaPreviewCard({ obra, navigate }) {
  const mapRef = React.useRef(null);
  const mapInstance = React.useRef(null);
  const coords = parseLocalizacao(obra['LOCAL DA OBRA']);
    ...
    <div className="card border-0 shadow-sm">
      <div className="card-body">
        ...
        {coords ? (
          ...
          <p className="small text-muted mb-0">Localização não cadastrada para esta obra. Edite a obra e preencha o campo Local da Obra para habilitar o mapa.</p>
        )}
      </div>
    </div>
  );
