 ...
 * ============================================================
 *  MapaPage.js
 *  Mapa com um pin por obra que tenha o campo Localização
...
function MapaPage({ navigate, params }) {
  ...
  const obrasComPin = React.useMemo(
    () => obras.filter((o) => parseLocalizacao(o['Localização'])),
    [obras]
  );
  ...
  React.useEffect(() => {
    ...
    obrasComPin.forEach((obra) => {
      const [lat, lng] = parseLocalizacao(obra['Localização']);
      ...
      {!loading && obrasComPin.length === 0 && (
        <div className="mt-3">
          <EmptyState icon="bi-geo-alt" title="Nenhuma obra com localização cadastrada"
            subtitle="Edite uma obra e preencha o campo Localização (manualmente ou usando o botão de GPS) para que ela apareça aqui." />
        </div>
      )}
    </div>
  );
...
function MapaPreviewCard({ obra, navigate }) {
  const mapRef = React.useRef(null);
  const mapInstance = React.useRef(null);
  const coords = parseLocalizacao(obra['Localização']);
    ...
    <div className="card border-0 shadow-sm">
      <div className="card-body">
        ...
        {coords ? (
          ...
          <p className="small text-muted mb-0">Localização não cadastrada para esta obra. Edite a obra e preencha o campo Localização para habilitar o mapa.</p>
        )}
      </div>
    </div>
  );
