...
const MENU_ITEMS = [
  ...
  { key: 'carteira', label: 'Carteira', icon: 'bi-briefcase' },
  { key: 'mapa', label: 'Mapa', icon: 'bi-geo-alt' }
];

function Sidebar({ currentPage, navigate, collapsed, onCloseMobile }) {
  const { session } = useSession();
  const menuItems = usuarioPodeGerenciarUsuarios(session)
    ? [...MENU_ITEMS, { key: 'usuarios', label: 'Usuários', icon: 'bi-people' }]
    : MENU_ITEMS;
      ...
      <aside className={`sidebar ${collapsed ? 'collapsed' : ''}`}>
        ...
        {!collapsed && (
          <div className="sidebar-footer">
            ...
            <small className="text-muted d-block">v2.1.0</small>
          </div>
        )}
      </aside>
...
function Topbar({ onToggleSidebar, theme, onToggleTheme, onSearch, pageTitle }) {
  ...
  const handleGlobalRefresh = async () => {
    setRefreshing(true);
    try {
      await Promise.all([DataStore.refreshAll(), syncAllStores()]);
